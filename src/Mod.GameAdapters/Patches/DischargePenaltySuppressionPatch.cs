...
namespace Enlisted.Mod.GameAdapters.Patches
{
    ...
    public class DischargeRelationPenaltyPatch
    {
        ...
        public static bool Prefix(Hero hero, Hero gainedRelationWith, int relationChange, bool showQuickNotification)
        {
            try
            {
                ...
                ModLogger.Debug("Discharge",
                    $"RelationChange intercepted: player<->{otherHeroName}, change={relationChange}, isDischarging={DischargeState.IsDischarging}, showNotification={showQuickNotification}");
                ...
                if (DischargeState.IsDischarging && relationChange < 0)
                {
                    ...
                    ModLogger.Info("Discharge",
                        $"Suppressed relation penalty of {relationChange} during discharge (OtherHero: {otherHeroName})");
    ...
    public static class DischargeHelper
    {
        ...
        public static void RestoreKingdomWithoutPenalties(Clan clan, Kingdom targetKingdom)
        {
            ...
            ModLogger.Debug("Discharge",
                $"RestoreKingdomWithoutPenalties called: clan={clanName}, current={currentKingdomName}, target={targetKingdomName}");
            ...
            try
            {
                ...
                ModLogger.Debug("Discharge",
                    "DischargeState.IsDischarging set to TRUE - relation penalties will be suppressed");
                ...
                try
                {
                    ...
                    ModLogger.Debug("Discharge",
                        $"Current state: currentKingdom={currentKingdom?.Name?.ToString() ?? "null"}, isMercenary={isMercenary}");
                ...
                finally
                {
                    ...
                    ModLogger.Debug("Discharge",
                        "DischargeState.IsDischarging set to FALSE - relation penalties restored to normal");
                }
            }
            catch (Exception ex)
            {

    ...
    public class EnlistmentExpenseIsolationPatch
    {
        ...
        public static MethodBase TargetMethod()
        {
            try
            {
                ...
                if (method != null)
                {
                    ModLogger.Info("ExpenseIsolation",
                        $"Found AddPartyExpense method for patching: {method.DeclaringType?.Name}.{method.Name}");
                    return method;
                }
        ...
        public static bool Prefix(MobileParty party, Clan clan, ExplainedNumber goldChange, bool applyWithdrawals,
            ref int __result)
        {
            try
            {
                ...
                if (!_loggedFirstInvocation)
                {
                    ...
                    ModLogger.Info("ExpenseIsolation",
                        $"Patch active - AddPartyExpense intercepted (applyWithdrawals={applyWithdrawals})");
                }

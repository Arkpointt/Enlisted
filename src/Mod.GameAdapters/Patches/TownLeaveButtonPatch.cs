...
{
    /// <summary>
    /// Harmony patch that hides the native "Leave" button in town menus when the player
        ...
        static void Postfix(ref bool __result, MenuCallbackArgs args)
        {
            try
            {
                ...
                if (enlistment?.IsEnlisted == true && 
                    ...
                    !enlistment.IsInGracePeriod)
                {
                    __result = false;
                    ModLogger.Debug("TownLeave", "Hiding native Leave button - player is actively enlisted");
                }
            }
            catch (Exception ex)
            {

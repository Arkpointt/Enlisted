			...
			if (lordKingdom != null && playerClan != null)
			{
				if (playerClan.Kingdom != lordKingdom)
				{
					try
					{
						...
						ChangeKingdomAction.ApplyByJoinFactionAsMercenary(playerClan, lordKingdom, default(CampaignTime), 0, false);
						ModLogger.Info("Enlistment", $"Joined {lordKingdom.Name} as mercenary while enlisted with {lord.Name}");
					}
					catch (Exception ex)
					{
				...
				if (hero == _enlistedLord)
				{
					...
					if (settlement.IsTown || settlement.IsCastle)
					{
						...
						if (!inBattleOrSiege)
						{
							...
							if (MobileParty.MainParty != null && Hero.MainHero != null && CampaignEventDispatcher.Instance != null)
							{
								try
								{
								...
								catch (Exception ex)
								{
									...
									ModLogger.Warn("Settlement", $"Synthetic perk trigger suppressed error: {ex.Message}");
								}
							}

...
{
    /// <summary>
    /// Lance facts: wounded/sick/dead/replacements deltas and a light training tag (best-effort).
    /// </summary>
    public sealed class LanceStatusFactProducer : IDailyReportFactProducer
    {
        public void Contribute(DailyReportSnapshot snapshot, CampNewsContext context)
        {
            ...
            try
            {
                ...
                // Training tag (best-effort): read today’s schedule if available.
                var schedule = context.Schedule?.CurrentSchedule;
                if (schedule?.Blocks != null && schedule.Blocks.Any(b => b != null && b.BlockType == ScheduleBlockType.TrainingDrill))
                {
                    snapshot.TrainingTag = TrainingTag.Drilling;
                }
                else
                {

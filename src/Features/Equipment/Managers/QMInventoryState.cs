...
namespace Enlisted.Features.Equipment.Managers
{
    ...
    /// </summary>
    public sealed class QMInventoryState
    {
        /// <summary>
        ...
        /// </summary>
        [SaveableProperty(1)]
        public Dictionary<string, int> CurrentStock { get; } = new Dictionary<string, int>();
        ...
        /// <summary>
        /// Muster cycle duration in days.
        /// </summary>
        private const int RefreshCycleDays = 12;

        /// <summary>
        /// Current campaign day as an integer.
        /// </summary>
        private int CurrentDay => (int)CampaignTime.Now.ToDays;
        ...
        public bool NeedsRefresh()
        {
            ...
            // Handle edge case of time going backwards or initial state
            return daysSinceRefresh >= RefreshCycleDays || daysSinceRefresh < 0;
        }
        ...
        {
            // Clamp supply level early to ensure consistency between logic and metadata
            ...
            try
            {
                ...
                var itemCount = (int)MathF.Max(1f, availableItems.Count * varietyPercent);

                // Efficiently shuffle and select items to stock
                var itemsToStock = new List<ItemObject>(availableItems);
                for (var i = 0; i < itemsToStock.Count; i++)
                {
                    var randomIndex = MBRandom.RandomInt(i, itemsToStock.Count);
                    (itemsToStock[i], itemsToStock[randomIndex]) = (itemsToStock[randomIndex], itemsToStock[i]);
                }

                var selectedItems = itemsToStock.Take(itemCount);
                ...
                ModLogger.Info("Inventory", $"Stocked {itemCount} items (variety: {varietyPercent:P0}, qty range: {minQty}-{maxQty})");
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="supplyLevel">The supply level used for the refresh.</param>

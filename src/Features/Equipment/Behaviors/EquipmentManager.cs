    ...
    public sealed class EquipmentManager : CampaignBehaviorBase
    {
        ...
        public int CalculateEquipmentCost(CharacterObject troop, FormationType formation)
        {
            try
            {
                ...
                var formationMultiplier =
                    _formationPriceMultipliers.TryGetValue(formation, out var fMult) ? fMult : 1.0f;
                var cultureMultiplier = _culturePriceMultipliers.TryGetValue(troop.Culture.StringId, out var cMult)
                    ? cMult
                    : 1.0f;
        ...
        public void BackupPersonalEquipment()
        {
            try
            {
                ...
                _personalBattleEquipment =
                    hero.BattleEquipment.Clone(); // Default cloneWithoutWeapons=false is sufficient
                _personalCivilianEquipment =
                    hero.CivilianEquipment.Clone(); // Default cloneWithoutWeapons=false is sufficient
        ...
        [SuppressMessage("ReSharper", "UnusedMember.Global",
            Justification = "May be used for future equipment selection features")]
        ...
        /// </summary>
        [SuppressMessage("ReSharper", "UnusedMember.Global", Justification = "May be called from menu system")]
            ...
            try
            {
                ...
                if (availableTroops is { Count: > 0 })
                {
                    ...
                    if (Hero.MainHero.Gold >= cost)
                    {
                        ...
                        ModLogger.Info("Gold",
                            $"Equipment purchased: {selectedTroop.Name} for {cost} denars (had {goldBefore}, now {Hero.MainHero.Gold})");
                    ...
                    {
                        ModLogger.Warn("Gold",
                            $"Insufficient funds for equipment: need {cost} denars, have {Hero.MainHero.Gold}");

    ...
    public sealed class QuartermasterManager : CampaignBehaviorBase
    {
        ...
        public void RequestEquipmentVariant(ItemObject requestedItem, EquipmentIndex slot)
        {
            try
            {
                ...
                ModLogger.Debug("Quartermaster",
                    $"Limit check: {requestedItem.Name} in {slot} - owned={currentCount}, limit={itemLimit} ({(itemLimit == 1 ? "armor" : "weapon")})");
                ...
                if (currentCount >= itemLimit)
                {
                    var limitMsg = itemLimit == 1
                        ...
                        : new TextObject(
                            "{=qm_item_limit_reached_2}Two's the limit, soldier. Army regs. You already have {COUNT} of {ITEM_NAME}.");
                    ...
                    ModLogger.Info("Quartermaster",
                        $"Item limit reached: {requestedItem.Name} (count: {currentCount}, limit: {itemLimit})");
                    return;
                }
        ...
        private void BuildQuartermasterStatusDisplay()
        {
            try
            {
                ...
                var qmDialogue =
                    new TextObject(
                        "{=qm_intro_dialogue}\"Take what you need, soldier. The army provides. Mind your kit - anything missing gets docked from your pay.\"");

...
namespace Enlisted.Features.Equipment.Behaviors
{
    ...
    public sealed class QuartermasterManager : CampaignBehaviorBase
    {
        ...
        private readonly List<ReturnOption> _returnOptions = new();
        ...
        // ReSharper disable once NotAccessedField.Local - Field is assigned for future conversation-based equipment selection
        private Dictionary<int, EquipmentVariantOption> _conversationEquipmentVariants = new();

        // Conversation tracking for dynamic equipment selection
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future conversation-based equipment selection")]
        private Dictionary<int, EquipmentVariantOption> _conversationWeaponVariants = new();
        ...
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future cache invalidation")]
        ...
        public Dictionary<EquipmentIndex, List<ItemObject>> GetTroopEquipmentVariants(CharacterObject selectedTroop)
        {
            try
            {
                ...
                ModLogger.Info("Quartermaster",
                    $"Discovered {variants.Sum(kvp => kvp.Value.Count)} equipment variants for {selectedTroop.Name}");
                return variants;
            }
            catch (Exception ex)
            {
        ...
        public CharacterObject GetPlayerSelectedTroop()
        {
            try
            {
                ...
                {
                    ModLogger.Info("Quartermaster",
                        $"Player troop identified as: {selectedTroop.Name} ({formation} formation)");
                }
        ...
        public int CalculateVariantCost(ItemObject requestedItem, ItemObject currentItem, EquipmentIndex slot)
        {
            try
            {
                ...
                var finalCost = (int)(costDifference * slotMultiplier * formationMultiplier * discountMultiplier +
                                      serviceFee);
        ...
        public void RequestEquipmentVariant(ItemObject requestedItem, EquipmentIndex slot)
        {
            try
            {
                ...
                if (currentCount >= MaxItemsPerType)
                {
                    var limitMsg =
                        new TextObject(
                            "{=qm_item_limit_reached}Two's the limit, soldier. Army regs. You already have {COUNT} of {ITEM_NAME}.");
                    ...
                    ModLogger.Info("Quartermaster",
                        $"Item limit reached: {requestedItem.Name} (count: {currentCount})");
                    return;
                }
                ...
                if (IsWeaponSlot(slot) || IsConsumableItem(requestedItem))
                {
                    ...
                    if (targetSlot == EquipmentIndex.None)
                    {
                        ...
                        if (partyInventory != null)
                        {
                            ...
                            var inventoryMsg =
                                new TextObject("{=qm_added_to_inventory}{ITEM_NAME} stowed in your pack. Hands full.");
                            inventoryMsg.SetTextVariable("ITEM_NAME", requestedItem.Name);
                            InformationManager.DisplayMessage(new InformationMessage(inventoryMsg.ToString(),
                                Colors.Yellow));
                            ModLogger.Info("Quartermaster",
                                $"Weapon slots full - {requestedItem.Name} added to inventory");
                        }
                        else
                        {
                            ...
                            var noSlotsMsg =
                                new TextObject(
                                    "{=qm_no_weapon_slots}Your hands are full, soldier. Return something to the armory first.");
                            InformationManager.DisplayMessage(new InformationMessage(noSlotsMsg.ToString(),
                                Colors.Yellow));
                ...
                var successMessage =
                    new TextObject("{=qm_equipment_issued}Equipment issued: {ITEM_NAME}. Army provides.");
                ...
                ModLogger.Info("Quartermaster",
                    $"Equipment issued: {requestedItem.Name} to slot {targetSlot} (replaced {previousItemName})");
            }
            catch (Exception ex)
            {
                ModLogger.Error("Quartermaster",
                    $"Error processing equipment variant request for {requestedItem?.Name?.ToString() ?? "null"} in slot {slot}: {ex.Message}",
                    ex);
            }
        }
        ...
        private void OnQuartermasterEquipmentInit(MenuCallbackArgs args)
        {
            try
            {
                ...
                ModLogger.Info("Quartermaster",
                    $"Quartermaster menu opened for {_selectedTroop.Name} with {_availableVariants?.Count ?? 0} variant slots");
            }
            catch (Exception ex)
            {
            ...
            Dictionary<EquipmentIndex, List<ItemObject>> variants)
        {
            ...
            {
                ModLogger.Info("Quartermaster",
                    "Applying quartermaster officer enhancements - expanding equipment access");

                // Add culture-wide equipment variants for quartermaster officers
                var cultureVariants =
                    GetCultureEquipmentVariants(enlistment?.CurrentLord?.Culture, enlistment?.EnlistmentTier ?? 1);
        ...
        /// </summary>
        private Dictionary<EquipmentIndex, List<ItemObject>> GetCultureEquipmentVariants(CultureObject culture,
            int maxTier)
        {
            try
            {
                ...
                ModLogger.Info("Quartermaster",
                    $"Officer enhancement: Added {variants.Sum(kvp => kvp.Value.Count)} culture-wide equipment options");
                return variants;
            }
            catch (Exception ex)
            {
        ...
        private void BuildQuartermasterStatusDisplay()
        {
            try
            {
                ...
                var qmDialogue = new TextObject(
                    "{=qm_intro_dialogue}\"Take what you need, soldier - the army provides. Two of each, no more. Mind you, any gear that goes missing will be deducted from your pay when you change posts. Keep your kit in order.\"");
        ...
        ///     Create dynamic menu options for each equipment slot with variants.
        /// </summary>
        [SuppressMessage("ReSharper", "UnusedParameter.Local", Justification = "Required by menu callback signature")]
        ...
        private void OnQuartermasterVariantsInit(MenuCallbackArgs args)
        {
            try
            {
                ...
                if (_selectedSlot == EquipmentIndex.None ||
                    !_availableVariants.TryGetValue(_selectedSlot, out var options))
                {
                    MBTextManager.SetTextVariable("VARIANT_TEXT",
                        "No equipment variants available for the selected slot.");
                    return;
                }
        ...
        private void OnQuartermasterReturnsInit(MenuCallbackArgs args)
        {
            try
            {
                ...
                var sb = new StringBuilder();
                sb.AppendLine(
                    new TextObject("{=qm_return_intro}Return issued gear to the armory. Select an item to hand back.")
                        .ToString());
                sb.AppendLine();
        ...
        [SuppressMessage("ReSharper", "UnusedMember.Global",
            Justification = "May be called from menu system or other modules")]
        ...
        private void OnWeaponVariantsSelected(MenuCallbackArgs args)
        {
            try
            {
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=qm_no_weapon_variants}No weapon variants available for your troop tree.")
                            .ToString()));
                    return;
                }
        ...
        private void ShowEquipmentVariantSelectionDialog(List<EquipmentVariantOption> variants, string equipmentType)
        {
            try
            {
                ...
                {
                    ModLogger.Info("Quartermaster",
                        $"Opened Gauntlet equipment selector for {equipmentType} with {variants.Count} variants");
                }
                else
                {
        ...
        private bool TryShowGauntletEquipmentSelector(List<EquipmentVariantOption> variants, string equipmentType)
        {
            try
            {
                ...
                QuartermasterEquipmentSelectorBehavior.ShowEquipmentSelector(variants, targetSlot, equipmentType);
        ...
        /// </summary>
        private void ShowConversationBasedEquipmentSelection(List<EquipmentVariantOption> variants,
            string equipmentType)
        {
            try
            {
                ...
                if (availableVariants.Count > 1)
                {
                    ...
                    _conversationEquipmentVariants =
                        availableVariants.ToDictionary(v => availableVariants.IndexOf(v) + 1, v => v);
        ...
        private void ShowSimplifiedVariantSelection(List<EquipmentVariantOption> variants, string equipmentType)
        {
            try
            {
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject($"No affordable {equipmentType} variants available. You need more gold.")
                            .ToString()));
                }
            }
            catch (Exception ex)
        ...
        private bool IsArmorVariantsAvailable(MenuCallbackArgs args)
        {
            ...
            try
            {
                ...
                var armorSlots = new[]
                {
                    EquipmentIndex.Body, EquipmentIndex.Head, EquipmentIndex.Leg, EquipmentIndex.Gloves
                };
        ...
        private void OnArmorVariantsSelected(MenuCallbackArgs args)
        {
            try
            {
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=qm_no_armor_variants}No armor variants available for your troop tree.")
                            .ToString()));
                    return;
                }
        ...
        /// </summary>
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future helmet variant selection")]
        [SuppressMessage("ReSharper", "UnusedParameter.Local", Justification = "Required by menu callback signature")]
        ...
        private Dictionary<EquipmentIndex, List<EquipmentVariantOption>> BuildArmorOptionsFromCurrentTroop()
        {
            ...
            try
            {
                ...
                ModLogger.Debug("Quartermaster",
                    $"BuildArmorOptions: Tier={tier}, Culture={culture?.Name}, SelectedTroop={selectedTroop.Name}");
                ...
                foreach (var node in branchNodes)
                {
                    ...
                    ModLogger.Debug("Quartermaster",
                        $"  Branch node: {node.Name} (Tier {SafeGetTier(node)}, Loadouts: {loadoutCount})");
                }
                ...
                if (tier >= 4)
                {
                    ...
                    ModLogger.Debug("Quartermaster",
                        $"BuildArmorOptions: Tier {tier} - using all branch tiers for variety");
                ...
                else
                {
                    ...
                    {
                        ModLogger.Debug("Quartermaster",
                            $"BuildArmorOptions: No armor at exact tier {tier}, expanding to all branch tiers");
                        armorVariants = CollectArmorVariantsFromAllTiers(branchNodes, culture);
                    }
                }
                ...
                ModLogger.Debug("Quartermaster",
                    $"BuildArmorOptions: Final result has {result.Count} slots with options");
            }
            catch (Exception ex)
            {
        ...
        private Dictionary<EquipmentIndex, List<EquipmentVariantOption>> BuildWeaponOptionsFromCurrentTroop()
        {
            ...
            try
            {
                ...
                ModLogger.Debug("Quartermaster",
                    $"BuildWeaponOptions: Tier={tier}, Culture={culture?.Name}, SelectedTroop={selectedTroop.Name}");
                ...
                if (tier >= 4)
                {
                    ...
                    ModLogger.Debug("Quartermaster",
                        $"BuildWeaponOptions: Tier {tier} - using all branch tiers for variety");
                ...
                else
                {
                    ...
                    {
                        ModLogger.Debug("Quartermaster",
                            $"BuildWeaponOptions: No weapons at exact tier {tier}, expanding to all branch tiers");
                        weaponVariants = CollectWeaponVariantsFromAllTiers(branchNodes, culture);
                    }
                }
                ...
                ModLogger.Debug("Quartermaster",
                    $"BuildWeaponOptions: Final result has {result.Count} slots with options");
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        private Dictionary<EquipmentIndex, List<ItemObject>> CollectWeaponVariantsFromNodes(
            HashSet<CharacterObject> nodes, int exactTier, CultureObject culture)
        {
            var variants = new Dictionary<EquipmentIndex, List<ItemObject>>();
            try
            {
        ...
        /// </summary>
        private Dictionary<EquipmentIndex, List<ItemObject>> CollectWeaponVariantsFromAllTiers(
            HashSet<CharacterObject> nodes, CultureObject culture)
        {
            var variants = new Dictionary<EquipmentIndex, List<ItemObject>>();
            try
            {
                ...
                foreach (var troop in nodes)
                {
                    ...
                    ModLogger.Debug("Quartermaster",
                        $"  Scanning weapons: {troop.Name?.ToString() ?? "Unknown"} ({loadoutCount} loadouts)");
                    ...
                    foreach (var equipment in troop.BattleEquipments)
                    {
                        foreach (var slot in weaponSlots)
                        {
                            ...
                            {
                                ModLogger.Debug("Quartermaster",
                                    $"    {slot}: {item.Name} ({weaponType}) - SKIP (culture: {itemCulture})");
                                continue;
                            }
                            ...
                            if (!variants[slot].Contains(item))
                            {
                                ...
                                ModLogger.Debug("Quartermaster",
                                    $"    {slot}: {item.Name} ({weaponType}){shieldTag} - ADDED");
                            }
                        }
                    }
                }
        ...
        /// </summary>
        private Dictionary<EquipmentIndex, List<ItemObject>> CollectArmorVariantsFromAllTiers(
            HashSet<CharacterObject> nodes, CultureObject culture)
        {
            var variants = new Dictionary<EquipmentIndex, List<ItemObject>>();
            try
            {
                var armorSlots = new[]
                {
                    EquipmentIndex.Body, EquipmentIndex.Head, EquipmentIndex.Gloves, EquipmentIndex.Leg,
                    EquipmentIndex.Cape
                };
                foreach (var troop in nodes)
                {
                    ...
                    var loadoutCount = troop.BattleEquipments.Count();
                    ModLogger.Debug("Quartermaster",
                        $"  Scanning {troop.Name?.ToString() ?? "Unknown"} ({loadoutCount} loadouts)");
                    ...
                    foreach (var equipment in troop.BattleEquipments)
                    {
                        foreach (var slot in armorSlots)
                        {
                            ...
                            {
                                ModLogger.Debug("Quartermaster",
                                    $"    {slot}: {item.Name} - SKIP (culture mismatch: {itemCulture} vs {culture.Name})");
                                continue;
                            }
                            ...
                            if (!variants[slot].Contains(item))
                            {
                                variants[slot].Add(item);
                                ModLogger.Debug("Quartermaster",
                                    $"    {slot}: {item.Name} - ADDED (culture: {itemCulture})");
                            }
                        }
                    }
                }
        ...
        private HashSet<CharacterObject> BuildTroopBranchNodes(CultureObject culture, CharacterObject targetTroop,
            int maxTier)
        {
        ...
        private Dictionary<EquipmentIndex, List<ItemObject>> CollectArmorVariantsFromNodes(
            HashSet<CharacterObject> nodes, int exactTier, CultureObject culture)
        {
            var variants = new Dictionary<EquipmentIndex, List<ItemObject>>();
            try
        ...
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future exact variant matching")]
        ...
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future armor slot selection UI")]
        ...
        private void OnHelmetVariantsSelected(MenuCallbackArgs args)
        {
            try
            {
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=qm_no_helmet_variants}No helmet variants available for your troop type.")
                            .ToString()));
                }
            }
            catch (Exception ex)
        ...
        private void OnFoodOptimizationSelected(MenuCallbackArgs args)
        {
            ...
            if (foodBonus > 0)
            {
                ...
                var message =
                    new TextObject(
                        "{=qm_optimized_food}Optimized food supplies: +{AMOUNT} grain from efficient rationing.");
        ...
        private void OnInventoryManagementSelected(MenuCallbackArgs args)
        {
            ...
            var message =
                new TextObject("{=qm_reorganized_inventory}Reorganized inventory: +{AMOUNT} temporary carry capacity.");
        ...
        private void OnSupplyPurchaseSelected(MenuCallbackArgs args)
        {
            ...
            if (Hero.MainHero.Gold >= cost)
            {
                GiveGoldAction.ApplyBetweenCharacters(Hero.MainHero, null,
                    cost); // Default disableNotification=false is sufficient
        ...
        [SuppressMessage("ReSharper", "UnusedParameter.Local",
            Justification = "Required by menu callback signature - parameter name indicates it's intentionally unused")]
        private void OnVariantOptionSelected(MenuCallbackArgs _, int optionIndex)
        {
            try
            {
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=qm_already_have_variant}You already have this equipment variant.")
                            .ToString()));
                    return;
                }
        ...
        /// </summary>
        [SuppressMessage("ReSharper", "UnusedMember.Local",
            Justification = "May be used for future variant text formatting")]

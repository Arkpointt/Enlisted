    ...
    public sealed class QuartermasterManager : CampaignBehaviorBase
    {
        ...
        public void RequestEquipmentVariant(ItemObject requestedItem, EquipmentIndex slot)
        {
            try
            {
                ...
                // Charge the player
                GiveGoldAction.ApplyBetweenCharacters(hero, null,
                ...
                ModLogger.Info("Quartermaster",
                    $"Equipment variant applied: {requestedItem.Name} to slot {slot} for {cost} denars (replaced {previousItemName})");
            }
            catch (Exception ex)
            {
            ...
            string equipmentType)
        {
            try
            {
                var availableVariants =
                    variants.Where(v => !v.IsCurrent).Take(5).ToList(); // Limit to 5 for conversation
        ...
        private void OnSupplyManagementInit(MenuCallbackArgs args)
        {
            try
            {
                ...
                sb.AppendLine(
                    $"Current Inventory: {party.TotalWeightCarried:F1}/{party.InventoryCapacity:F1} capacity");

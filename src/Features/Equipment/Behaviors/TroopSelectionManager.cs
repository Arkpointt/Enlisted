...
namespace Enlisted.Features.Equipment.Behaviors
{
    ...
    {
        private List<CharacterObject> _availableTroops = new();
        ...
        private Dictionary<int, IssuedItemRecord> _issuedEquipment = new();
            ...
            try
            {
                ...
                if (enlistment?.IsEnlisted != true)
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=eq_must_be_enlisted}You must be enlisted to use Master at Arms.")
                            .ToString()));
                    return;
                }
                ...
                {
                    InformationManager.DisplayMessage(new InformationMessage(
                        new TextObject("{=eq_no_eligible_troops}No eligible troops found for your rank and culture.")
                            .ToString()));
                    return;
                }
                ...
                var data = new MultiSelectionInquiryData(
                    ...
                    selected =>
                    {
                        try
                        {
                            ...
                            {
                                ModLogger.Info("TroopSelection",
                                    $"Player selected troop: {chosen.Name} (ID: {chosen.StringId}, Tier: {SafeGetTier(chosen)}, Formation: {DetectTroopFormation(chosen)})");
                                ...
                                {
                                    Campaign.Current.GameMenuManager.RefreshMenuOptions(Campaign.Current
                                        .CurrentMenuContext);
                                }
                            }
                        }
                ...
                _ =>
                {
                    NextFrameDispatcher.RunNextFrame(() =>
                    {
                        try
                        {
                            ...
                            ModLogger.Info("TroopSelection",
                                "Player chose to collect equipment now - opening Master at Arms");
                        }
                        catch (Exception ex)
                        {
            ...
            starter.AddGameMenuOption("enlisted_troop_selection", "troop_selection_back",
                ...
                _ =>
                {
                    NextFrameDispatcher.RunNextFrame(() =>
                    {
                        ...
                        {
                            ModLogger.Error("TroopSelection",
                                $"Failed to switch back to enlisted status: {ex.Message}");
        ...
        public void ShowTroopSelectionMenu(int newTier)
        {
            try
            {
                ...
                ModLogger.Info("TroopSelection",
                    $"Troop selection menu opened - {_availableTroops.Count} troops available");
            }
            catch (Exception ex)
            {
        ...
        private void OnTroopSelectionInit(MenuCallbackArgs args)
        {
            try
            {
                ...
                statusText +=
                    $"\"Soldier, your service has not gone unnoticed. You've earned promotion to {rankName}.\"\n\n";
            ...
            catch (Exception ex)
            {
                ...
                MBTextManager.SetTextVariable("TROOP_SELECTION_TEXT",
                    "Error loading troop selection. Please report this issue.");
            }
        }
        ...
        public List<CharacterObject> GetTroopsForCultureAndTier(string cultureId, int tier)
        {
            try
            {
                ...
                ModLogger.Info("TroopSelection",
                    $"Found {availableTroops.Count} tree troops for {cultureId} tier {tier}");
                ...
                while (queue.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        if (upgrades != null)
                        {
                            ...
                            {
                                if (next != null && next.Culture == culture && !next.IsHero &&
        ...
        public void ApplySelectedTroopEquipment(Hero hero, CharacterObject selectedTroop)
        {
            try
            {
                ...
                var message =
                    new TextObject("{=eq_promoted_new_equipment}Promoted to {TROOP_NAME}! New equipment issued.");
            ...
            {
                ModLogger.Error("TroopSelection",
                    $"Failed to apply selected troop equipment for {selectedTroop?.Name?.ToString() ?? "null"}: {ex.Message}",
                    ex);
            }
        }
        ...
        private void ProcessEquipmentAccountability(Hero hero)
        {
            try
            {
                ...
                if (totalDebt > 0)
                {
                    ...
                    var headerText =
                        new TextObject("{=qm_missing_equipment_header}Missing equipment deducted from pay:");
                    ...
                    var chargeMsg =
                        new TextObject(
                            "{=qm_missing_equipment_charge}Missing equipment charge: {AMOUNT} denars deducted from pay.");
                    ...
                    ModLogger.Info("TroopSelection",
                        $"Charged {totalDebt} denars for {missingItems.Count} missing items");
                }
        ...
        public void RecordIssuedEquipment(TaleWorlds.Core.Equipment equipment)
        {
            try
            {
                ...
                ModLogger.Info("TroopSelection",
                    $"Recorded {_issuedEquipment.Count} issued equipment items for accountability");
            }
            catch (Exception ex)
            {
        ...
        public (List<IssuedItemRecord> MissingItems, int TotalDebt) CheckMissingEquipment()
        {
            ...
            try
            {
                ...
                {
                    ModLogger.Info("TroopSelection",
                        $"Found {missingItems.Count} missing items worth {totalDebt} denars");
                }
            }
            catch (Exception ex)
            {

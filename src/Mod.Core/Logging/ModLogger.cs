...
namespace Enlisted.Mod.Core.Logging
{
    ...
    public static class ModLogger
    {
        ...
        private static Dictionary<string, LogLevel> _categoryLevels = new(StringComparer.OrdinalIgnoreCase);
        private static LogLevel _defaultLevel = LogLevel.Info;

        // Throttling system - tracks recent messages to prevent spam
        private static readonly Dictionary<string, ThrottleEntry> _throttleCache = new();
        ...
        private static readonly Dictionary<string, SummaryEntry> _summaryData = new();
        ...
        public static void Initialize(string customPath = null)
        {
            ...
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Debug.WriteLine(
                    $"[Enlisted] Logger initialization failed: {ex.Message}\n{ex.StackTrace}");
        ...
        public static void ConfigureLevels(Dictionary<string, LogLevel> categoryLevels,
            LogLevel defaultLevel = LogLevel.Info, int throttleSeconds = 5)
        {
            lock (Sync)
            {
                ...
                WriteInternal("INFO", "Init",
                    $"Log levels configured: default={defaultLevel}, throttle={_throttleSeconds}s, categories={_categoryLevels.Count}");
            }
        }
        ...
        /// <param name="level">Log level (default Info)</param>
        public static void FlushSummary(string key, string category, string messageFormat,
        ...
        private static void LogWithThrottle(string level, string category, string message)
        {
            ...
            lock (Sync)
            {
                ...
                else
                {
                    ...
                    {
                        LastLogTime = now, SuppressedCount = 0, Level = level, Category = category
                    };
                    WriteInternal(level, category, message);
                }
            }
        ...
        private static void WriteInternal(string level, string category, string message)
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine(
                            $"[Enlisted] Primary log path failed, using fallback: {fallback} (Error: {ex.Message})");
                ...
                catch (Exception fallbackEx)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine(
                        $"[Enlisted] Both primary and fallback log paths failed. Primary: {ex.Message}, Fallback: {fallbackEx.Message}");
                }
            }
        }
        ...
        private static string ResolveDefaultLogPath()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(assemblyLocation))
                {
                    ...
                    System.Diagnostics.Debug.WriteLine(
                        "[Enlisted] Assembly.Location is null/empty, using Documents fallback");
                ...
                {
                    System.Diagnostics.Debug.WriteLine(
                        "[Enlisted] Could not get directory from assembly location, using Documents fallback");
                    return ResolveDocumentsPath("enlisted.log");
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine(
                        "[Enlisted] Could not get parent directory, using Documents fallback");
                    return ResolveDocumentsPath("enlisted.log");
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine(
                        "[Enlisted] Could not get module root directory, using Documents fallback");
                    return ResolveDocumentsPath("enlisted.log");
                }
                ...
                System.Diagnostics.Debug.WriteLine(
                    $"[Enlisted] Resolved path doesn't look correct: {logPath}, using Documents fallback");
                return ResolveDocumentsPath("enlisted.log");
            }
            catch (Exception ex)
            {
                ...
                System.Diagnostics.Debug.WriteLine(
                    $"[Enlisted] Failed to resolve log path: {ex.Message}\n{ex.StackTrace}");
                return ResolveDocumentsPath("enlisted.log");
            }
        }

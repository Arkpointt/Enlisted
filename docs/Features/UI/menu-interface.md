...
**Why No Tick Restoration:** For army members, native code uses `UnstoppableFastForward` when the user clicks fast
forward. If tick handlers restore `CapturedTimeMode` whenever they see `UnstoppableFastForward`, the next tick
immediately reverts user input, breaking speed controls.

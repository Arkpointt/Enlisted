# Feature Specs

Detailed documentation for each major feature.

## Features

- [enlistment.md](enlistment.md) - Core service: wages, XP, kills, retirement, grace periods
- [temporary-leave.md](temporary-leave.md) - 14-day leave with desertion penalties
- [quartermaster.md](quartermaster.md) - Equipment selection UI
- [dialog-system.md](dialog-system.md) - Conversation management
- [troop-selection.md](troop-selection.md) - Real troop choice for promotions
- [encounter-safety.md](encounter-safety.md) - Map encounter crash prevention
- [duties-system.md](duties-system.md) - Military roles and assignments
- [formation-training.md](formation-training.md) - Formation-based skill XP
- [battle-commands.md](battle-commands.md) - Formation-based command filtering
- [menu-interface.md](menu-interface.md) - Menu system
- [town-access-system.md](town-access-system.md) - Settlement exploration

## Spec Structure

Each spec covers:
- **Overview**: One sentence summary
- **Purpose**: Why it exists
- **Behavior**: How it works
- **Technical**: Key files and APIs
- **Edge Cases**: What can go wrong

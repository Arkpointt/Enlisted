...
# Tool budget tracking per task
_TOOL_BUDGET = {
    "enabled": True,
    "max_calls": 15,  # Default max tool calls per task
    "current_calls": 0,
    "exceeded": False,
}


def set_tool_budget(max_calls: int = 15):
    """Set the maximum tool calls allowed per task."""
    _TOOL_BUDGET["max_calls"] = max_calls
    _TOOL_BUDGET["current_calls"] = 0
    _TOOL_BUDGET["exceeded"] = False
    _TOOL_BUDGET["enabled"] = True


def reset_tool_budget():
    """Reset tool call counter (call at start of each task/crew)."""
    _TOOL_BUDGET["current_calls"] = 0
    _TOOL_BUDGET["exceeded"] = False


def disable_tool_budget():
    """Disable tool budget enforcement."""
    _TOOL_BUDGET["enabled"] = False


def get_tool_budget_status() -> dict:
    """Get current tool budget status."""
    return _TOOL_BUDGET.copy()


@before_tool_call
def enforce_tool_budget(context):
    """
    Enforce tool call budget to prevent runaway exploration.
    
    Returns False to block execution when budget is exceeded.
    """
    if not _TOOL_BUDGET["enabled"]:
        return None  # Budget enforcement disabled
    
    _TOOL_BUDGET["current_calls"] += 1
    current = _TOOL_BUDGET["current_calls"]
    max_calls = _TOOL_BUDGET["max_calls"]
    
    tool_name = getattr(context, "tool_name", "unknown")
    
    if current > max_calls:
        if not _TOOL_BUDGET["exceeded"]:
            # First time exceeding - log warning
            _TOOL_BUDGET["exceeded"] = True
            print(f"\n      [⚠️ BUDGET EXCEEDED] Tool call {current}/{max_calls} blocked: {tool_name}")
            print(f"      Agent should now synthesize from gathered information.\n")
        return False  # Block the tool call
    
    # Log progress toward budget
    if current == max_calls - 2:
        print(f"      [📊 BUDGET] {current}/{max_calls} calls used - 2 remaining")
    elif current == max_calls:
        print(f"      [📊 BUDGET] {current}/{max_calls} calls used - LAST CALL")
